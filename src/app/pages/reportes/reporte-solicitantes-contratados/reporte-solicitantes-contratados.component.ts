...
declare var $: any;
@Component({
  selector: 'app-reporte-solicitantes-contratados',
  templateUrl: './reporte-solicitantes-contratados.component.html',
  styles: [
  ]
})
export class ReporteSolicitantesContratadosComponent implements OnInit {
  ...
  public reporteForm = this.fb.group({
    ...
    num_contrataciones: [1, [Validators.required, Validators.min(1)]]
  });
  ...
  filtrar(): void {
    ...
    this.reporteService.generarListadoSolicitantesContratados(formularioCompleto).subscribe(({contrataciones, ocupacion}) => {
       this.contrataciones = contrataciones;
       this.ocupacion = ocupacion;
       this.total = this.contrataciones.length;
       Swal.fire(
        'Filtrado exitoso!',
        '',
        'success'
  ...
  generarPDF(): void{
    ...
    doc.setFontSize(20);
    doc.text(90, 40, 'Reporte de Solicitantes contratados');
    ...
    doc.save('reporte-solicitantes-contratados.pdf');
  }
  obtenerFechas(): void {
    const fechas = $('#daterange7').val();

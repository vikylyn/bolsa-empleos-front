...
@Component({
  selector: 'app-reporte-solicitantes-rechazados',
  templateUrl: './reporte-solicitantes-rechazados.component.html',
  styles: [
  ]
})
export class ReporteSolicitantesRechazadosComponent implements OnInit {
 ...
 constructor(private fb: FormBuilder,
  ...
  public reporteForm = this.fb.group({
    ...
    num_rechazos: [1, [Validators.required, Validators.min(1)]]
  });

  ngOnInit(): void {
    $('#daterange6').daterangepicker(
      {
        locale : {
          format : 'DD-MM-YYYY',
          separator : ' / ',
  ...
  filtrar(): void {
    ...
    this.reporteService.generarListadoPostulacionesRechazadas(formularioCompleto).subscribe(({postulaciones, ocupacion, total}) => {
       this.postulaciones = postulaciones;
       this.ocupacion = ocupacion;
       this.total = this.postulaciones.length;
       Swal.fire(
        'Filtrado exitoso!',
        '',
        'success'
  ...
  generarPDF(): void{
    ...
    doc.setFontSize(20);
    doc.text(90, 40, 'Reporte de Solicitantes rechazados');
    ...
    doc.save('reporte-solicitantes-rechazados.pdf');
  }
  obtenerFechas(): void {
    const fechas = $('#daterange6').val();
